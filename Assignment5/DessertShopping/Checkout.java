...
public class Checkout {
    ...
    public String toString() {
        ...
        for (DessertItem item : items) {
            ...
            if (itemClass.contains("IceCream")) {
                ...
                receipt += ((Candy) item).getWeight() + " lbs. @ " +
                        DessertShoppe.cents2dollarsAndCents(((Candy) item).getPriceInCents()) + " /1b.\n";
                while (s1.length() < DessertShoppe.maxLengthOfItemName) {
                    s1 += " ";
                }
                receipt += s1 + itemPayment + "\n";
        ...
        while (l.length() <= DessertShoppe.recepitWidth - tax.length()) {
            l += " ";
        }
        receipt += l + tax;
        ...
        l = "\nTotal Cost";
        while ((l.length() <= DessertShoppe.recepitWidth - totalPayment.length())){
            l += " ";
        }

        receipt += l + totalPayment;
